...
if [ ! -f Pathfinder.java ]; then
	echo ""
	echo " Error: You must place Pathfinder.java in this directory before we can"
...
function check_maze_files()
{
	local maze_file=$1
	local path_file=$2

	if [ ! -f input_files/$maze_file ]; then
		echo ""
		echo " Error: You must place $maze_file in the input_files directory"
		echo "        before we can proceed. Aborting test script."
		echo ""
		exit
	fi

	if [ ! -f input_files/$path_file ]; then
		echo ""
		echo " Error: You must place $path_file in the input_files directory"
		echo "        before we can proceed. Aborting test script."
		echo ""
		exit
	fi
...
for i in `seq -f "%02g" 1 $NUM_INPUT_FILES`;
do
	check_maze_files "maze$i.txt" "maze$i-all-paths.txt"
done
...
function run_test_case()
{
	...
	# Attempt to compile.
	javac Pathfinder.java $testcase.java 2> /dev/null
	compile_val=$?
