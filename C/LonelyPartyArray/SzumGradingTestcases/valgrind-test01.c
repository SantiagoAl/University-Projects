...
// ===================================
// LonelyPartyArray: valgrind-test01.c
// ===================================
// A copy of testcase04.c, used to test for memory leaks.
...
int main(void)
{
	...
	for (i = 0; i < num_fragments; i++)
	{
		if (i != 4 && party->fragment_sizes[i] != 0)
			failwhale("(Err1.09a) party->fragment_sizes[i] check failed.");
		if (i == 4 && party->fragment_sizes[i] != 1)
	...
	for (i = 0; i < fragment_length; i++)
	{
		...
		if (i == 2 && party->fragments[4][i] != 100)
	...
	if (party->fragment_sizes[4] != 1)
		failwhale("(Err2.04) party->fragment_sizes check failed.");
	if (party->fragments[4] == NULL)
